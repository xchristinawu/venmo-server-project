...
public class JdbcUserDao implements UserDao
{
    ...
    @Override
    public List<User> listAllOtherUsers(String principalName) {
        List<User> usersExceptMe = new ArrayList<>();
        String sql = "SELECT user_id, username FROM tenmo_user WHERE username NOT ILIKE ?";

        SqlRowSet results = jdbcTemplate.queryForRowSet(sql, principalName);
        while (results.next())
        {
            User user = new User();
            user.setId(results.getInt("user_id"));
            user.setUsername(results.getString("username"));
            usersExceptMe.add(user);
        }
